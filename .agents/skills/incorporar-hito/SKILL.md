...
7. **Importe (si el documento cuantifica dinero).** Si un `Hecho` derivado tiene una cifra económica clara y trazable al documento, rellena los campos `importe*` ([doc 01 §2.6](../../../docs/diseno/01-modelo-de-datos.md) + [ficha importe-presunto](../../../docs/web/features/importe-presunto.md)):
   ...
   - **Atribución por sujeto (`importe_atribucion`).** Los autos y sentencias suelen desglosar quién recibe/paga/sufre el dinero: rellena entonces `importe_atribucion[]` con el `papel` económico de cada sujeto (clase `objeto` → `activo` conducta atribuida / `beneficiario` percibe / `perjudicado` víctima; clase `consecuencia` → `obligado` paga / `acreedor` cobra). **`activo` ≠ percepción; el quebranto del `perjudicado` nunca se suma al investigado; `beneficiario` solo si el documento lo respalda.** Cada `sujeto` ∈ implicados (V-24); papel coherente con la clase (V-25). Tabla en [doc 01 §2.6](../../../docs/diseno/01-modelo-de-datos.md#26-hecho).
