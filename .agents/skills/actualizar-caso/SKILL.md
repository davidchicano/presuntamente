...
11. **Importe estructurado**. Si un Hecho nuevo cuantifica dinero trazable a su fuente, rellena `importe` + `importe_alcance` (obligatorio si hay importe, V-22) + `importe_naturaleza` ([doc 01 §2.6](../../../docs/diseno/01-modelo-de-datos.md), [ficha importe-presunto](../../../docs/web/features/importe-presunto.md)). **Anti-doble-conteo (V-23):** si la cifra ya está estructurada en otro Hecho, desglosa un total ya contabilizado, es una petición de pena no firme o un ofrecimiento no percibido → `componente` (no suma). Divisa extranjera → estructura la cifra relevante en euros y anota la original en `importe_nota`. Esto es además del texto libre de `sintesis_caso.cifras_clave`. **Atribución por sujeto:** si el hito nuevo trae reparto nominal del dinero (sentencia con indemnizaciones, auto con beneficiarios), rellena/actualiza `importe_atribucion[]` con el `papel` de cada sujeto (objeto: `activo`/`beneficiario`/`perjudicado`; consecuencia: `obligado`/`acreedor`). `activo` ≠ percepción; el quebranto del perjudicado nunca se suma al investigado; V-24 (sujeto ∈ implicados) y V-25 (papel↔clase) bloquean si hay incoherencia. Detalle en [doc 01 §2.6](../../../docs/diseno/01-modelo-de-datos.md#26-hecho).
