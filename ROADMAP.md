...
**Última actualización:** 2026-06-03 (**API de datos abiertos: diseño documentado, sin implementar**). A raíz de una petición pública de Menjòmetre (integrar los datos / "casos de Catalunya"), se diseñó y documentó la capa de datos abiertos **sin tocar código**: JSON estático sobre Cloudflare Pages (sin servidor), grafo caso/persona/organización, acceso índice+detalle+slices, partido como proyección de afectación (sin rankings), **filtro territorial resuelto en cliente** vía organizaciones (cruce por `cif`/NIF; modelar territorio queda diferido), sin SDK (`llms.txt`), propagación a terceros asumida. Canon: [`docs/api/`](docs/api/README.md) (contrato + `decisiones.md` D1-D10), ficha [`api-datos-abiertos.md`](docs/web/features/api-datos-abiertos.md), detalle en [`historial-2026-06.md`](docs/roadmap/historial-2026-06.md). **Pendiente de implementar** (ver backlog "API de datos abiertos"). `git push` pendiente.

**Anterior (2026-06-03).** Cierre de los 8 borradores restantes. Panel `/promover-caso` (18 evaluadores Sonnet ciegos) + re-panel (12) sobre los 5 paneleables + `pujol`: **6 → `beta_publica`** (`barcenas-caja-b`, `atico-estepona`, `filesa`, `punica`, `forum-filatelico`, `pujol`; autorizados por el maintainer). `eres-andalucia` y `tandem` se quedan en borrador **a propósito** (firmeza verificada no limpia: ERE suspendido esperando al TJUE; Tándem todo recurrible). Antes del panel: resueltas las **4 incertidumbres de `pujol`** (fecha_inicio→apertura 15-jun-2021, cohecho retirado, cuadro procesal completo con Gironès + 9 empresarios, fecha hito apertura) y la sugerencia de `malaya` (nivel_fuente 1→4). El panel cazó y se arreglaron **bugs reales** (ref rota + fase en punica, error factual del TC en un rol de filesa, V-13 en forum). `validate` 1382 OK; build limpio; 0 fugas. Detalle: [`historial-2026-06.md`](docs/roadmap/historial-2026-06.md). **`git push` pendiente** (lo decide el maintainer).
