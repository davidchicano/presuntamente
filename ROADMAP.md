...
**Última actualización:** 2026-06-04 (**coherencia web↔API en contenido retractado + limpieza de jerga interna en campos públicos**). (1) `PgCasoDetalle` ya **no renderiza hechos retractados** (`vigencia: retirado` / `estado_publicacion: retirado_*`), mismo criterio que la API — resuelve el follow-up de [D12](docs/api/decisiones.md#d12--decisiones-de-implementación-junio-2026); **conserva `superado`** (histórico). Borrado el único hecho retractado del repo (placeholder obsoleto de `tarjetas-black` que filtraba «Fichero obsoleto…» a ficha y `/graficas`). (2) Eliminada la coletilla interna **«ver NOTES.md»** de los campos renderizados (5 `estado_ficha.notas`, 19 `rol.notas` que salían en ficha y `/conexiones`, + 3 documentos y 2 corpus de cobertura por higiene); texto público autónomo, sin perder matiz. Verificado: `grep -r NOTES.md dist/` = 0, `Fichero obsoleto` = 0, hecho canónico y `superado` intactos; `validate` 0 err + build OK. Canon: [`visibilidad-estados-publicacion.md`](docs/web/features/visibilidad-estados-publicacion.md). **Sin pushear** (working tree commiteado).

**Anterior (2026-06-04).** **API de datos abiertos `/api/v1/`** (9 endpoints + `/llms.txt` + CORS; más estricta que la web: excluye retractados y `promocion_propuesta`; **CIF poblado 1→73**; suite `vitest` en CI) — canon [`docs/api/`](docs/api/README.md) + [`api-datos-abiertos.md`](docs/web/features/api-datos-abiertos.md). · **Explorador `/conexiones`**: `cose`→`fcose` (~10× más rápido) — [`explorador-conexiones.md`](docs/web/features/explorador-conexiones.md). Detalle de sesiones: [`historial-2026-06.md`](docs/roadmap/historial-2026-06.md).
...
- [x] **API de datos abiertos** — **implementada (v1, 2026-06-04)**. Endpoints estáticos `/api/v1/` (índice+detalle+slice de partido+dump+datapackage) + `/llms.txt` + `_headers`, gate `visibilidad.ts`, CIF inline + arista inversa, sobre `meta`. CIF poblado 1→73 (2 pasadas). Contrato + decisiones D1-D12 en [`docs/api/`](docs/api/README.md), ficha [`api-datos-abiertos.md`](docs/web/features/api-datos-abiertos.md). Origen: petición de Menjòmetre. Pendiente v1.x: 10 CIF sin fuente (`el-pais` ya resuelto; motivos en la ficha), valorar DIR3/Wikidata, verificar `_headers` en deploy CF real, posible página `/api`.
