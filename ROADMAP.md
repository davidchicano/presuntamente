...
**Última actualización:** 2026-06-04 (**fix de IDs duplicados en collections anidadas**). Las collections `roles`/`hitos`/`hechos` derivaban el id de Astro sólo de `data.id`, que es único **únicamente dentro de su caso** → dos casos con el mismo `data.id` (p. ej. Ruz `juez_instructor` en bárcenas-caja-b, gürtel y forum-filatelico) colisionaban y uno sobrescribía al otro: **pérdida silenciosa de datos en build** (el rol desaparecía de las fichas perdedoras). Fix en [`src/content.config.ts`](src/content.config.ts): el id de collection de las subentidades pasa a `<caso>/<data.id>` (helper `generateIdConCaso`). No cambia ningún `data.id`, anclas ni feed (la app filtra por `caso_id`); preempta el mismo bug en hitos/hechos; un duplicado real dentro de un caso sigue avisando. `build` 0 warnings (antes 9) · `check` 0 errores. Canon: comentario en el propio fichero.

**Anterior (2026-06-04).** **Regla 7 de afectación + `leire-diez` al día**: nueva regla 7 del [doc 08](docs/diseno/08-afectacion-directa-indirecta.md) (partido cuya caja/financiación es objeto del procedimiento → afectación **directa**, naturaleza `caja_partido_objeto_investigacion_en_caso`; aplicada a leire/filesa/bárcenas-caja-b/gürtel/palau, púnica y pujol siguen indirecta) + hito del levantamiento parcial del secreto en `leire-diez` (auto Pedraz 1-jun + CGPJ N1) + fix de `scripts/archivar-n4.mjs`. Antes: migración a Astro 6 (6.4.2). Detalle: [`NOTES` leire](content/casos/leire-diez/NOTES.md) · [doc 08 §"Aplicación regla 7"](docs/diseno/08-afectacion-directa-indirecta.md#aplicación-de-la-regla-7-2026-06-04) · [`historial-2026-06.md`](docs/roadmap/historial-2026-06.md).
