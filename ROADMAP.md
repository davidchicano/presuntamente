...
- **Fase activa:** **Post-launch temprano.** Fase 0, 1.0 y 1 cerradas; Fase 2 con **20 casos con contenido modelado** (12 en beta+ público · 8 en borrador). Sitio público, tráfico real.
- **Casos públicos (beta+) — 12:** `plus-ultra` (pin#1), `leire-diez` (pin#2), `begona-gomez`, `gonzalez-amador`, `fiscal-general-del-estado`, `kitchen`, `lezo`, `gurtel`, y **`noos` · `malaya` · `palau-musica` · `tarjetas-black`** (subidos 2026-06-02 vía `/promover-caso`, panel 3/3 VERDE + autorización).
- **Casos en borrador — 8:** `punica`, `pujol` (roles actualizados 2026-06-02: 7 hijos→`acusado`, Pujol Soley→`desimputado`; incertidumbres en su `NOTES.md`), `eres-andalucia`, `barcenas-caja-b`, `filesa`, `tandem`, `forum-filatelico`, `atico-estepona`.
...
- **Próximo paso:** **(1)** `git push` (lo decide el maintainer; `main` 10+ commits por delante). **(2)** panel `/promover-caso` sobre los 8 borradores restantes (`eres`/`tandem` se mantienen fuera hasta confirmar firmeza). **(3)** resolver las 4 incertidumbres de `pujol` (convención `fecha_inicio`, `cohecho` en Jordi hijo, Gironès/empresarios sin modelar, fecha del hito de apertura) antes de promoverlo. **(4)** `archive:catchup`; Zapatero 17-18 jun.
