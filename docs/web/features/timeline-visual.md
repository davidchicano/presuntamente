...
> Archivos clave: [`src/components/Hito.astro`](../../../src/components/Hito.astro) · [`src/components/pages/PgCasoDetalle.astro`](../../../src/components/pages/PgCasoDetalle.astro) (sección §2.6 + CSS `.timeline` y `.timeline-order`) · [`src/layouts/BaseLayout.astro`](../../../src/layouts/BaseLayout.astro) (event-delegation del toggle de orden)
...
3. **Colores por familia** — reusa los tokens del sistema de badges (DESIGN.md §"Sistema de badges", familia F-función):
...
Deep-link `#hito-<slug>` y target-flash `:target` (DESIGN.md §5) siguen funcionando intactos — la prop `id` de `Hito.astro` se sigue propagando como `id="hito-<slug>"` del contenedor `.hito-item`.
...
- **Sin glyph dentro del dot.** Se evaluó meter el mismo glyph del badge de categoría (⚖ jurisdiccional / ◆ político / ¶ mediático) dentro del dot. Se descartó: el badge de categoría ya lleva el glyph a la derecha; duplicarlo en el dot añade ruido sin información nueva. El color del dot ya basta para identificación rápida; el glyph del badge confirma a quien quiere verificar.
...
- [ ] Verificar el render en dark mode tras un cambio de paleta futuro. El rail usa `--color-border` y el dot tiene halo `--color-surface` — ambos tokens funcionan en ambos modos, así que en principio no hay trabajo, pero conviene revisar visualmente cuando se actualice DESIGN.md §"Tokens".
